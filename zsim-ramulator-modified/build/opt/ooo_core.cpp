...
// ////////////////////////////////////////////////////////////////
OOOCore::OOOCore(FilterCache* _l1i, FilterCache* _l1d, g_string& _name, bool only_offload_, bool instr_trace_, g_string& _outFile) : Core(_name), l1i(_l1i), l1d(_l1d), cRec(0, _name) {
...
inline void OOOCore::bbl_offload(/*Address bblAddr, BblInfo* bblInfo*/) {
  //print_trace = true;
	...
	switch (uop->type) {
			...
			if(!pim_trace) {
			  ...
			  offData.msg = INSTR_COUNT; // enter in the offloading funtion
                          offData.coreIdx = coreIdx;
			  offData.threadIdx = threadIdx;
			  offData.inter_instrs = previous_instr;
