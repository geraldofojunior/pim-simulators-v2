...
uint64_t MemoryTraces::access(MemReq& req) {
    ...
    switch (req.type) {
        case PUTX:
            ...
            //printf("writeback!\n");
            if(print_trace){
              //tracefile << "- - " << previous_instructions << " S " << req.lineAddr << std::endl;
              //tracefile << req.coreId << " " << req.threadId << " " << previous_instr[req.coreId] << " S " << req.readAddr << " " << req.lineAddr << std::endl; //to addresses is the original format of
	      futex_lock(&traceLock);        
	      tracefile << threadIdx[req.coreId] << " " << req.coreId << " " << previous_instr[req.coreId] << " S " << " " << req.lineAddr << std::endl; //to addresses is the original format of Ramulatori
        ...
        case GETS:
            *req.state = req.is(MemReq::NOEXCL)? S : E;
            ...
            if(print_trace){
	      futex_lock(&traceLock);        
              if(req.is(MemReq::PREFETCH_TRACE)) tracefile << threadIdx[req.coreId] << " " << req.coreId << " " << previous_instr[req.coreId] << " P " << req.lineAddr << std::endl;
              else tracefile  << threadIdx[req.coreId] << " " << req.coreId  << " " << previous_instr[req.coreId] << " L " << req.lineAddr << std::endl;
	      futex_unlock(&traceLock);        
            }
            previous_instr[req.coreId] = 0;
            break;
        case GETX:
            *req.state = M;
            ...
            if(print_trace){
	      futex_lock(&traceLock);        
              tracefile << threadIdx[req.coreId]<< " " << req.coreId << " " << previous_instr[req.coreId] << " L " << req.lineAddr << std::endl;
	      futex_unlock(&traceLock);        
            }
            previous_instr[req.coreId] = 0;
            break;
