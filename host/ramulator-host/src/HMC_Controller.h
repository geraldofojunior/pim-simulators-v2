...
public:
    ...
    vector<int> num_requests_per_core;
    vector<int> num_response_per_core;
    DRAM<HMC>* channel;
        ...
        refresh(new Refresh<HMC>(this))
    {
        ...
        num_requests_per_core.resize(4);
        num_response_per_core.resize(4);
        for(int i =0;i<4;i++) num_requests_per_core[i] = 0;
        for(int i =0;i<4;i++) num_response_per_core[i] = 0;
        if (with_drampower) {
    ...
    bool enqueue(Request& req)
    {
        ...
        if (queue.max == queue.size()){
            //cout << "queue max is equal to queue size at HMC Controller \n";
            return false;
        }
        if(req.type == Request::Type::READ ||req.type == Request::Type::WRITE)
          num_requests_per_core[req.coreid]++;
    ...
    Packet form_response_packet(Request& req) {
      ...
      if(req.coreid == 1) cout << "response \n";
      if(req.type == Request::Type::READ ||req.type == Request::Type::WRITE)
        num_response_per_core[req.coreid]++;
      return packet;
    }
