...
public:
    ...
    void set_is_stalled(){
       /* this->is_stalled = true;*/
        int rsqts[4];
        int rsp[4];
        for(int i=0;i<4;i++){rsqts[i] = 0; rsp[i]=0;}
        for (auto ctrl: ctrls){
            for(int i =0; i < 4; i++){
                rsqts[i] += ctrl -> num_requests_per_core[i];
                rsp[i] += ctrl -> num_response_per_core[i];
            }
        }
        cout << "Requests: ";
        for(int i=0;i<4;i++){
    ...
    int assign_tag(int slid) {
      if (tags_pools[slid].empty()) {
          //geraldo: I need to fix this (tag pool is not being filled up)
        return -1;
      } else {
    ...
    void receive_packets(Packet packet) {
      ...
      if(is_stalled){
         cout << "sending packet back to core " << packet.req.coreid << " via link " << packet.header.SLID.value << endl;
      }
    ...
    bool send(Request req)
    {
        ...
        if(coreid == 1) cout << "received request from core 1 \n";
        ...
        switch(int(type)) {
          ...
          break;
          default:
            cout << "Couldn't slice the address \n";
              assert(false);
        }
        ...
        if (packet.header.TAG.value == -1) {
          ...
          //cout << "Tag for link not available \n";
          return false;
        }
        ...
        if (packet.total_flits <= link->slave.available_space()) {
          ...
          return true;
            ...
            //cout << "No available space in link slave \n";
          return false;
        }
    }
