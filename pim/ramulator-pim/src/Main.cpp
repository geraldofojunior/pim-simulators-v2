...
void run_cputrace(const Config& configs, Memory<T, Controller>& memory, const std::vector<string>& files)
{
    ...
    if(configs.pim_mode_enabled()) cpu_tick = mem_tick;
...
int main(int argc, const char *argv[])
{
    ...
    desc.add_options()
      ...
      ("pim-cache-lines", po::value<string>(), "number of pim cache lines")
      ("pim-org", po::value<string>(), "pim organization")
      ("pim-core-count", po::value<string>(), "number of pim cores")
      ("trace_format", po::value<string>(), "trace format is row clone trace")
       ;
    po::variables_map vm;
    ...
    const string& trace_format = vm["trace_format"].as<string>();
    ...
    const string& pim_cache_lines = vm["pim-cache-lines"].as<string>();
    const string& pim_core_count = vm["pim-core-count"].as<string>();
    const string& pim_org = vm["pim-org"].as<string>();

    configs.set_pim_cache_lines(atoi(pim_cache_lines.c_str()));
    configs.set_pim_core_count(atoi(pim_core_count.c_str()));
    configs.set_pim_org(pim_org);
