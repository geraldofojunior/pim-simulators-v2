...
public:
    struct pim_cache{
        vector<long> tag;
        vector<bool> valid;
        vector<unsigned> lru_counter;
    };

    unsigned number_cache_lines;
    unsigned ways = 2;
    vector<pim_cache> pim_private_cache;
    long clk = 0;
    long retired = 0;
    int id = 0;
    int position;
    ...
    bool check_cache_hit(long);
    void update_lru(unsigned, unsigned);
    void insert_cache_line(long);
    void tick_inOrder();
    void tick_outOrder();
